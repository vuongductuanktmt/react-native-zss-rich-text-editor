...
export default class RichTextToolbar extends Component {
    ...
    _defaultRenderAction(action, selected) {
        ...
        return (<TouchableOpacity key={action} style={btnStyle} onPress={() => this._onPress(action)}>
    ...
    render() {
        const viewStyle = [{ height: 50, backgroundColor: "#D3D3D3", alignItems: "center" }, this.props.style];
        return (<View style={viewStyle}>
        <FlatList data={this.state.dataSet} numColumns={this.state.actions.length} renderItem={(item) => this._renderAction(item.item.action, item.item.selected)}/>
      </View>);
    }
    _onPress(action) {
        switch (action) {
